"use client";
import { KotaProps } from "@/lib/interfaces";
...
export default function Table() {
  const [userData, setUserData] = useState<KotaProps[]>([]);
  const [onlykota, setOnlykota] = useState<KotaProps>();
  const [modal, setModal] = useState(false);
  const column: TableColumn<KotaProps>[] = [
    {
      name: "no",
  ...
  const updateUser = async (data: KotaProps) => {
    setModal(true);
    setOnlykota(data);
  };
  ...
  const delUser = async (id: string) => {
    const token = localStorage.getItem("token");
    const response = await fetch(`http://localhost:8000/api/v1/kota/${id}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
  ...
  return (
    <>
      ...
      {modal && <Modal setIsOpenModal={setModal} dataKota={onlykota} action="Edit" />}
    </>
  );
}
