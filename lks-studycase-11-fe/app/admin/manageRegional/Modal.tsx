import { User, Vaccination, Regional } from "@/lib/interfacse";
import React, { ChangeEvent, Dispatch, SetStateAction } from "react";
import toast from "react-hot-toast";

export default function Modal({ action, setIsOpenModal, data }: { action: string; setIsOpenModal: Dispatch<SetStateAction<boolean>>; data?: Regional }) {
  const HandleSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/v1/regional", {
        method: "POST",
        headers: {
      ...
      <div className="bg-white max-w-xl w-full rounded-lg p-5 z-20">
        ...
        <form onSubmit={HandleSubmit} className="space-y-2">
          <div className="flex flex-col">
            <label className="text-base font-medium" htmlFor="province">
              province
            </label>
            <input defaultValue={data?.province} type="text" className="px-4 py-2 rounded-lg border-2 border-yellow-500 outline-none hover:border-yellow-600 focus:ring-2 focus:ring-yellow-200" name="province" />
          </div>
          <div className="flex flex-col">
            <label className="text-base font-medium" htmlFor="district">
              district
            </label>
            <input defaultValue={data?.district} type="text" className="px-4 py-2 rounded-lg border-2 border-yellow-500 outline-none hover:border-yellow-600 focus:ring-2 focus:ring-yellow-200" name="district" />
          </div>
