...
export default function AdminLogin() {
  ...
  const HandleSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/v1/auth/LoginUser", {
        method: "POST",
        headers: {
      ...
      <div className="max-w-xl w-full shadow-sm border-2 border-gray-500 rounded-lg p-4">
        ...
        <form onSubmit={HandleSubmit} className="space-y-2">
          <div className="flex flex-col">
            ...
            <input type="text" className="px-4 py-2 rounded-lg border-2 border-yellow-500 outline-none hover:border-yellow-600 focus:ring-2 focus:ring-yellow-200" name="username" />
          </div>
          <div className="flex flex-col">
