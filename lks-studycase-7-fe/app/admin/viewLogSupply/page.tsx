"use client";
import { LogSupplyProps } from "@/lib/utilities";
import React, { useState } from "react";

export default function ViewLogSupply() {
  const [data, setData] = useState<LogSupplyProps[]>([]);
  ...
  const HandleFilter = async () => {
    try {
      if (typeof window !== undefined) {
        ...
        const response = await fetch(`http://localhost:8000/api/admin/view-log-supply`, {
          method: "POST",
          headers: {
        ...
        {data ? (
          data.map((item, i) => (
            <div key={i} className="bg-slate-100 hover:bg-slate-200 border-2 border-blue-400 rounded-lg p-3">
              <p>
                <span className="font-semibold">Supplier name:</span> {item.supplier.username}
              </p>
              <p>
                <span className="font-semibold">Bahan name:</span> {item.bahan.name}
              </p>
              <p>
                <span className="font-semibold">Bahan Price:</span> {item.bahan.price}
              </p>
              <p>
                <span className="font-semibold">Quantity Used:</span> {item.qty}
              </p>
              <p>
                <span className="font-semibold">Created at:</span> {item.created_at}
              </p>
              <p>
                <span className="font-semibold">Updated at:</span> {item.updated_at}
              </p>
            </div>
          ))
        ) : (
