"use client";
import { LogUsageProps } from "@/lib/utilities";
import React, { useState } from "react";

export default function ViewLogSupply() {
  const [data, setData] = useState<LogUsageProps[]>([]);
  ...
  const HandleFilter = async () => {
    try {
      if (typeof window !== undefined) {
        ...
        const response = await fetch(`http://localhost:8000/api/admin/view-log-usage`, {
          method: "POST",
          headers: {
        ...
        {data ? (
          data.map((item, i) => (
            <div key={i} className="bg-slate-100 hover:bg-slate-200 border-2 border-blue-400 rounded-lg p-3">
              ...
              <p>
                <span className="font-semibold">Usage At:</span> {item.usage_at}
              </p>
            </div>
          ))
        ) : (
